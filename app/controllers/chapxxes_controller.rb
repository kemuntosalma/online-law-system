class ChapxxesController < ApplicationController
  ...
  def index
    if params[:search]
      ...
      @chapxxes = Chapxx.where(["law LIKE  ?","%#{params[:search]}%"])
      @chapxxes = Chapxx.where(["crime LIKE  ?","%#{params[:search]}%"])
  ...
  private
    ...
    def chapxx_params
      params.require(:chapxx).permit(:section_number, :crime, :law, :search)
    end
end
