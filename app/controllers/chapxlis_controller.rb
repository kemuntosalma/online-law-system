class ChapxlisController < ApplicationController
  ...
  def index
    if params[:search]
      ...
      @chapxlis = Chapxli.where(["law LIKE  ?","%#{params[:search]}%"])
      @chapxlis = Chapxli.where(["crime LIKE  ?","%#{params[:search]}%"])
  ...
  private
    ...
    def chapxli_params
      params.require(:chapxli).permit(:section_number, :crime, :law, :search)
    end
end
