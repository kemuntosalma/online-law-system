class ChapviisController < ApplicationController
  ...
  def index
    if params[:search]
      ...
      @chapviis = Chapvii.where(["law LIKE  ?","%#{params[:search]}%"])
      @chapviis = Chapvii.where(["crime LIKE  ?","%#{params[:search]}%"])
  ...
  private
    ...
    def chapvii_params
      params.require(:chapvii).permit(:section_number, :crime, :law, :search)
    end
end
